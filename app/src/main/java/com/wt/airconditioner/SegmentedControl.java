...
/**
 * Пилюля из нескольких кнопок: выбранная залита акцентом, невыбранные серые.
 *
 * Высота, размер текста и отступы приходят параметрами: они разные у
 * переключателя режима и переключателя уровня.
 */
final class SegmentedControl {

    /** Фон невыбранного сегмента. */
    private static final int UNSELECTED = 0xFF303030;
    /** Тонкая линия между сегментами — почти чёрная. */
     ...
     * Пилюля обновляется на каждый шаг каскада, тап и чтение уровней из машины —
     * пересборка стоила бы ~40 View, столько же Drawable и повторный инфлейт
     * векторных иконок на слабом железе головы, а меняется только индекс.
     */
    ...
    private static Segment[] build(LinearLayout container, Item[] items,
        ...
        for (int index = 0; index < items.length; index++) {
            ...
            /**
             * Сегменты равной ширины: пилюля цельная, и «Авто» занимает
             * столько же, сколько «Вручную».
             */
    ...
    /**
     * Разделитель между сегментами — волосяная линия в один пиксель. Рисуется
     * подложкой самого сегмента, а не зазором: сквозь зазор просвечивал бы фон,
     * который под переключателем то тёмный, то подсвеченный.
     */
     ...
     * соседние кнопки стыкуются встык. Форма от палитры не зависит и потому
     * задаётся один раз; заливку ставит applySelection.
     */
