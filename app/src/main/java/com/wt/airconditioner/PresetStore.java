...
final class PresetStore {
     ...
     * сохранённые до появления запрета, уже лежат в хранилище, а адресация по
     * encode() на них неоднозначна.
     */
    List<Preset> load() {
    ...
    /**
     * false — точно такая запись уже есть. Отказ, а не второй экземпляр: их не
     * различить на экране, а адресация по encode() перестала бы быть однозначной.
     */
    ...
    /**
     * Заменяет пресет, сохраняя его место в списке. false — такого больше нет.
     *
     * Адресация строкой, а не индексом: между открытием редактора и сохранением
     * список могли перебрать, и индекс указал бы на чужую запись. Однозначность
     * обеспечивает add().
     */
