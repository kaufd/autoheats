...
- Не использовать `print` для критичных диагностических данных в релизе без флага — но текущая кодовая база использует `print` повсеместно, и переезд на logger пока не делался.
